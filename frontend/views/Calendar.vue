		...
		<div class="flex w-full flex-col">
			<div class="flex w-full justify-content items-center" v-for="year in years">
				<div class="flex items-center">
					<router-link :to="{ name: 'calendar.year', params: { year: year.label } }" class="flex-1 h-full p-2">{{ year.label }}</router-link>
				</div>
				<div class="mx-1 flex items-center flex-grow">
					<router-link :class="getHeatMap(item.value)" :title="item.value" :to="{ name: 'calendar.month', params: { year: year.label, month: item.slug } }" class="flex-1 border m-1 p-3" v-for="item in year.items"></router-link>
				</div>
			</div>
		</div>
	</div>
